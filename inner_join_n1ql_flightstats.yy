...
ontime2carrier:
	ontime_table INNER JOIN carrier_table ON_KEYS ( previous_table.`carrier` = current_table.`code` );

first2carrier:
	INNER JOIN carrier_table ON_KEYS ( a1.`carrier` = current_table.`code` );

ontime2airport:
	ontime_table INNER JOIN airport_table ON_KEYS ( previous_table. origin_destination = current_table.`code` ) ;

first2airport:
	INNER JOIN airport_table ON_KEYS ( a1. origin_destination = current_table.`code` );
	
origin_destination:
	`origin` | `destination` ;

ontime2aircraft:
	ontime_table INNER JOIN aircraft_table ON_KEYS ( current_table.`tail_num` = previous_table.`tail_num` ) ;

first2aircraft:
	INNER JOIN aircraft_table ON_KEYS ( a1.`tail_num` = current_table.`tail_num` );

airport2state:
	airport_table INNER JOIN state_table ON_KEYS ( previous_table.`state` = current_table.`state_code` );

airport2zipcode:
	airport_table INNER JOIN zipcode_table ON_KEYS ( previous_table.`state` = current_table.`state_code` );

airport2remark:
	airport_table INNER JOIN remark_table ON_KEYS ( `site_number` ) ;

aircraft2engine:
	aircraft_table INNER JOIN engine_table ON_KEYS ( `aircraft_engine_code` ) ;
